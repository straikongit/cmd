...
def cmd():
    ...
    if request.method == 'POST':
        ...
        s= [{}] 
        s[0][u"id"] = int(request.form["id"])
        s[0][u"header"] = request.form["header"]
        s[0][u"command"] = request.form["command"]
        s[0][u"tags"] = request.form["tags"]
        s[0][u"source"] = request.form["source"]
        ...
        for c in d["commands"]:
            if c["id"]== int(request.form["id"]):
                ...
                c[u"command"] = request.form["command"]
                c[u"tags"] = request.form["tags"]
                c[u"source"] = request.form["source"]
                found=True
                break
        if not found:
            d["commands"].extend(s)
